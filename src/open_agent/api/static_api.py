...
<div class="main-content">
    <div class="header">
        <div class="header-left">
            ...
            <button class="header-nav-btn" id="navSkillBtn" onclick="switchView('skill')">Skills</button>
    ...
    <div class="view-container" id="viewCron">
        <div class="panel-view-header">
            ...
            <button class="btn btn-sm send-button" onclick="toggleAddCronPanel()">+ Create Task</button>
        </div>
        ...
        <div class="add-cron-panel" id="addCronPanel" style="display: none;">
            ...
            <div class="form-row" style="justify-content: flex-end; margin-top: 4px; gap: 12px;">
                ...
                <button class="btn btn-sm send-button" onclick="submitCronTask()">Commit To Queue</button>
            </div>
        </div>
    ...
    <div class="view-container" id="viewMcp">
        <div class="panel-view-header">
            ...
            <button class="btn btn-sm send-button" onclick="toggleAddMcpPanel()">+ Register Server</button>
        </div>
        ...
        <div class="add-cron-panel" id="addMcpPanel" style="display: none;">
            ...
            <div class="form-row" id="mcpNetworkRow">
                <div class="form-group" style="flex: 2;">
                    ...
                    <input type="text" id="mcpUrl" class="form-control mono" placeholder="http://exp.wind.com.cn/.../mcp">
                </div>
                <div class="form-group" style="flex: 1;">
                    <label>HTTP Headers Context (JSON Format)</label>
                    <input type="text" id="mcpHeaders" class="form-control mono" placeholder='{"wind.sessionid": "xx"}'>
                </div>
            </div>
            ...
            <div class="form-row" style="justify-content: flex-end; margin-top: 4px; gap: 12px;">
                ...
                <button class="btn btn-sm send-button" onclick="submitMcpServer()">Commit Registry</button>
            </div>
        </div>
    ...
    <div class="view-container" id="viewConfig">
        <div class="panel-view-header">
            ...
            <button class="btn btn-sm send-button" onclick="toggleAddProviderPanel()">+ Inject Provider</button>
        </div>
        ...
        <div id="configFormContainer" style="display: flex; flex-direction: column; gap: 24px;">
            ...
            <div class="config-section-card">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                    ...
                    <button class="btn btn-sm send-button" style="height: 32px;" onclick="saveActiveModelRoute()">Save Routing Choice</button>
                </div>
                <div class="form-row">
                    <div class="form-group">
            ...
            <div class="add-cron-panel" id="addProviderPanel" style="display: none;">
                <h3 class="panel-view-title" style="font-size: 12px; color: var(--charcoal-700); margin-bottom: 4px;">+ Add Model Provider</h3>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-row" style="justify-content: flex-end; margin-top: 4px; gap: 12px;">
                    ...
                    <button class="btn btn-sm send-button" style="height: 36px;" onclick="submitNewProvider()">Inject Layer</button>
                </div>
            </div>
    ...
    <div class="input-wrapper" id="globalInputWrapper">
        <button class="workspace-btn" id="workspaceBtn" onclick="selectWorkspace()" title="Set Directory Context">
        ...
        <div class="input-container">
            ...
            <button class="interrupt-button btn" id="interruptButton" onclick="interruptConversation()" style="display: none;">Halt</button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        manualPathInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') handleManualJump();
        });

        // 装载会话历史并构建一个崭新的 Trace 环境
    ...
    function showConfirmDialog({title, text, onConfirm}) {
        closeConfirmDialog(); // 先行做边界清理安全机制
    ...
    async function fetchCronTasks() {
        cronListContainer.innerHTML = '<div style="padding:20px; text-align:center; font-family:var(--font-mono); color:var(--slate-400)">POLLING CRON REGISTRY...</div>';
        try {
    ...
    async function fetchSkillData() {
        skillListContainer.innerHTML = '<div style="padding:20px; text-align:center; font-family:var(--font-mono); color:var(--slate-400)">POLLING SKILL EXTRACTIONS...</div>';
        try {
    ...
    // ==========================================
    // 7. MCP (Model Context Protocol) 模块矩阵
    // ==========================================
                ...
                card.innerHTML = `
                    <div class="info-card-summary" onclick="toggleCardOpen(this.parentNode)">
                        ...
                        <div style="display:flex; gap:8px; align-items:center;" onclick="event.stopPropagation();">
                            <button class="btn btn-sm send-button" style="height:28px; padding:0 8px; font-size:10px;" onclick="updateSingleMcp('${name}')">Apply</button>
    ...
    async function testMcpServerTools(name) {
        ...
        if (!card.hasAttribute('open')) toggleCardOpen(card);
    ...
    async function fetchGlobalSettings() {
        ...
        try {
            ...
            globalProvidersCachedList.forEach(provider => {
                ...
                if (provider.name === selectRoute.model_provider) opt.selected = true;
                activeProviderSelect.appendChild(opt);
            });

            syncModelDropdown(selectRoute.model);
            ...
            card.innerHTML = `
                <div class="info-card-summary" onclick="toggleCardOpen(this.parentNode)">
                    ...
                    <div style="display:flex; gap:12px; align-items:center;" onclick="event.stopPropagation();">
                        <button class="btn btn-sm send-button" style="height:28px; padding:0 8px; font-size:10px;" onclick="updateSingleProvider('${name}')">Apply Cluster</button>
    ...
    function syncModelDropdown(targetSelectedModel = null) {
        ...
        if (activeProviderSelect.options.length === 0) return;
    ...
    async function saveActiveModelRoute() {
        ...
        const bodyPayload = {
            model_provider: providerSelect.value,
    ...
    // ==========================================
    // 9. 工作空间 (CWD) 及目录层级导航引擎
    // ==========================================
    async function loadDirList(path) {
        const dirListContainer = document.getElementById('dirList');
        const pathDisplay = document.getElementById('currentPathDisplay');
        dirListContainer.innerHTML = '<div style="padding:20px; text-align:center; font-family:var(--font-mono); font-size:12px; color:var(--slate-400)">LOADING...</div>';
        try {
    ...
    async function initDefaultWorkspace() {
        try {
            ...
            const data = await response.json();
            if (data.current_path) updateWorkspaceUI(data.current_path);
        } catch (e) {
            // 默认静默降级逻辑
        }
    }
    ...
    function handleManualJump() {
        ...
        if (targetPath) loadDirList(targetPath);
    }
        ...
        try {
            ...
            conversations.forEach(conversation => {
                ...
                item.innerHTML = `
                    ...
                    <button class="delete-btn" onclick="event.stopPropagation(); confirmDeleteConversation('${conversation.id}', '${escapeHtml(conversation.title)}')">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                `;
                item.onclick = () => {
    ...
    async function loadConversation(conversationId) {
        ...
        try {
            ...
            const data = await response.json();
            updateWorkspaceUI(data.work_dir);
            chatContainer.innerHTML = '';
            emptyState.style.display = 'none';

            if (data.messages && data.messages.length > 0) {
                const messages = processMessages(data.messages);
                messages.forEach(msg => {
                    if (msg.role === 'user') {
                        addUserMessage(msg.content, msg.time);
                    } else if (msg.role === 'assistant') {
                        addAssistantGroup(msg.content, msg.time);
                    }
                });
            } else {
            ...
            addUserMessage('ERR: Failed to load conversation', getCurrentTime());
        }
    }

    function processMessages(messages) {
        const msgs = JSON.parse(JSON.stringify(messages));
        // 将包装错误的 User Array 数据转换为 Assistant 角色进行合并
        msgs.forEach(msg => {
            if (msg.role === 'user' && Array.isArray(msg.content)) msg.role = 'assistant';
        });
        ...
        showConfirmDialog({
            ...
            onConfirm: async () => {
                try {
                    ...
                    if (String(currentConversationId) === String(conversationId)) startNewChat();
    ...
    function addAssistantGroup(contentArray, time) {
        emptyState.style.display = 'none';
        ...
        contentArray.forEach(block => {
            if (block.type === 'thinking') {
                htmlStr += `<details class="think-details" open><summary>${FOLD_SVG} Thought Process</summary><div class="content">${formatMarkdown(block.thinking)}</div></details>`;
            } else if (block.type === 'text') {
                htmlStr += `<div class="reply-content">${formatMarkdown(block.text)}</div>`;
            } else if (block.type === 'tool_use') {
                htmlStr += `<details class="tool-details" open><summary>${FOLD_SVG} Call: ${escapeHtml(block.name)}</summary><div class="content"><code>${escapeHtml(JSON.stringify(block.input))}</code></div></details>`;
            } else if (block.type === 'tool_result') {
                const status = block.is_error ? 'Error' : 'Result';
                htmlStr += `<details class="tool-details" open><summary>${FOLD_SVG} Tool ${status}</summary><div class="content"><code>${escapeHtml(typeof block.content === 'string' ? block.content : JSON.stringify(block.content))}</code></div></details>`;
            }
        });
        htmlStr += `<div class="message-time">${time}</div>`;
    ...
    function setTyping(typing) {
        ...
        interruptButton.style.display = typing ? 'flex' : 'none';
        sendButton.style.display = typing ? 'none' : 'flex';
    }

    async function handleStreamEnd() {
        setTyping(false);
        if (currentTargetNode) currentTargetNode.classList.remove('streaming-active');
        if (currentWrapper && !currentWrapper.querySelector('.message-time')) {
            currentWrapper.insertAdjacentHTML('beforeend', `<div class="message-time">${getCurrentTime()}</div>`);
        }
        currentWrapper = null;
        ...
        if (!currentConversationId) {
            ...
            if (firstConversation) currentConversationId = firstConversation.dataset.id;
        }
    ...
    async function interruptConversation() {
        ...
        if (currentWrapper && !currentWrapper.querySelector('.reply-content.error-hint')) {
            currentWrapper.insertAdjacentHTML('beforeend', `<div class="reply-content error-hint" style="color: var(--danger-color); font-size: 12px; font-family: var(--font-mono)">[CONNECTION INTERRUPTED BY USER]</div>`);
        }
    ...
    async function sendMessage() {
        ...
        addUserMessage(message, getCurrentTime());
        messageInput.value = '';
        autoResize();
        setTyping(true);

        let streamUrl = currentConversationId ? `/agent/stream/${currentConversationId}` : '/agent/stream';
        streamUrl += `?query=${encodeURIComponent(message)}&work_dir=${encodeURIComponent(currentWorkdir)}`;
        ...
        currentEventSource.onmessage = (event) => {
            ...
            if (!currentWrapper && data.type !== 'delta') {
                currentWrapper = document.createElement('div');
                currentWrapper.className = 'assistant-message';
                chatContainer.appendChild(currentWrapper);
            }
            ...
            if (data.type === 'thinking' || data.type === 'text' || data.type === 'tool_use' || data.type === 'tool_result') {
                ...
                if (data.type === 'thinking') {
                    ...
                    currentWrapper.appendChild(details);
                    currentTargetNode = details.querySelector('.content');
                } else if (data.type === 'text') {
                    const div = document.createElement('div');
                    div.className = 'reply-content streaming-active';
                    currentWrapper.appendChild(div);
                    currentTargetNode = div;
                ...
                if (currentRawText && currentTargetNode) currentTargetNode.innerHTML = formatMarkdown(currentRawText);
                scrollToBottomIfAtBottom();
            } else if (data.type === 'delta') {
                currentRawText += (data.text || '');
                if (currentTargetNode) currentTargetNode.innerHTML = formatMarkdown(currentRawText);
                scrollToBottomIfAtBottom();
            }
        };

        currentEventSource.onerror = async (e) => {
